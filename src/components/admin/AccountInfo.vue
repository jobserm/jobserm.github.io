<template>
  <div>
    ...
    <c-stack w="100%" my="5rem">
      ...
      <c-flex ml="2rem">
        <c-avatar
          v-if="user.name === undefined || user.lastname === undefined"
          size="2xl"
        />
        <c-avatar
          v-if="user.name !== undefined || user.lastname !== undefined"
          v-bind:name="user.name + ' ' + user.lastname"
          size="2xl"
        />
        <c-stack ml="3rem">
          <c-text v-if="user.name === undefined || user.lastname === undefined"
            >ชื่อ - นามสกุล: {{ "-" }}</c-text
          >
          <c-text v-if="user.name !== undefined || user.lastname !== undefined"
            >ชื่อ - นามสกุล: {{ user.name + " " + user.lastname }}</c-text
          >
          ...
          <c-text>
            Activation Status:
            <c-button v-if="user.activation == undefined" mx="2">{{
              "-"
            }}</c-button>
            <c-button
              v-if="user.activation == 0"
              mx="2"
              variant-color="green"
              height="30px"
              width="100px"
              @click="activationStatus"
              >{{ "ACTIVATED" }}</c-button
            >
            <c-button
              v-if="user.activation == 1"
              mx="2"
              variant-color="red"
              height="30px"
              width="100px"
              @click="activationStatus"
              >{{ "DEACTIVATED" }}</c-button
            >
          </c-text>
        </c-stack>
      </c-flex>
    ...
    async activationStatus() {
      ...
      try {
        ...
        let res = await backendInstance.get(
          `/api/users/${this.user.id}/toggle-activation`,
        );
        if (res.status === 200) {
